...
local north_biter_force_name = "north_biters"


event.on_init(function ()
	this.rocket_silos = {}
	this.spawn_generated = false
end)
...
local function get_noise(name, pos)
	...
	if name == 1 then
		...
		local final_noise = noise[1] + noise[2] * 0.08 + noise[3] * 0.025
		return final_noise
	end
	if name == 2 then
		local noise = {}
		...
		local final_noise = noise[1] + noise[2] * 0.2
		return final_noise
	end
	if name == 3 then
		local noise = {}
		...
		seed = seed + noise_seed_add
		noise[2] = simplex_noise(pos.x * 0.08, pos.y * 0.08, seed)
		local final_noise = noise[1] + noise[2] * 0.1
		return final_noise
	end
end
...
local function generate_circle_spawn(event)
	if this.spawn_generated then return end
...
local function generate_north_silo(surface)
	...
	this.rocket_silos["north"] = surface.create_entity({
		name = "rocket-silo",
		position = pos,
		force = "north"
	})
	this.rocket_silos["north"].minable = false

	for i = 1, 32, 1 do
		create_tile_chain(surface, {name = "stone-path", position = this.rocket_silos["north"].position}, 32, 10)
	end
end
	...
	if math_random(1, 160) == 1 then
		...
		if spawner_position then
			if math_random(1, 4) == 1 then
				surface.create_entity({name = "spitter-spawner", position = spawner_position, force = north_biter_force_name})
			else
				surface.create_entity({name = "biter-spawner", position = spawner_position, force = north_biter_force_name})
			end
		end
	end
end
...
local function on_chunk_generated(event)
	...
	for _, e in pairs(surface.find_entities_filtered({area = event.area, force = "enemy"})) do
		surface.create_entity({name = e.name, position = e.position, force = north_biter_force_name, direction = e.direction})
		e.destroy()
	end
	...
	if game.tick > 0 then
		...
		for _, silo in pairs(this.rocket_silos) do
...
local function setup (surface_name, silos)
	this.rocket_silos = silos
	local surface = game.surfaces[surface_name]
	if Config.on_init_pregen then
